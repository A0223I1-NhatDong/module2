...
public class PhoneService {
   ...
   public List<Phone> findByName(boolean isAuthentic, String name){
        if(isAuthentic){
           return authenticPhones.stream().filter(e-> e.getName().equals(name)).collect(Collectors.toList());
     }

//        return handPhones.stream().filter(e-> e.getName().equals(name)).collect(Collectors.toList());

        if(isAuthentic){
            List<Phone> result = new ArrayList<>();
            for (int i = 0; i < authenticPhones.size(); i++) {
                if(authenticPhones.get(i).getName().equals(name)){
        ...
        List<Phone> result = new ArrayList<>();
        for (int i = 0; i < handPhones.size(); i++) {
            if(handPhones.get(i).getName().equals(name)){
